...
import {getResultByUserIdInterviewIdFetch, cleargetResultByUserIdInterviewIdInfo} from "../../redux/toolkit/getResultByUserIdInterviewIdSlice.js";
...
const CompanyDashboard = () => {
	...
	const {updateInterviewStatusInfo, updateInterviewStatusError, updateInterviewStatusProgress, updateInterviewStatusInit} = useSelector((state) => state.updateInterviewStatusSlice);
	const {getResultByUserIdInterviewIdInfo, getResultByUserIdInterviewIdProgress} = useSelector((state) => state.getResultByUserIdInterviewIdSlice);
	console.log("getResultByUserIdInterviewIdProgress", getResultByUserIdInterviewIdProgress);
	console.log("getResultByUserIdInterviewIdInfo", getResultByUserIdInterviewIdInfo);
			...
			},
			{
				...
				render: (_, record) => (
					<Button
						...
						onClick={() => {
							const user = intervieweeListInfo.find((interviewee) => interviewee.user_id === record.user_id);
							console.log("user", user);
							handleShowResults(user.interview_id, user.user_id);
						}}
						// disabled={() => {
						// 	const user = intervieweeListInfo.find((interviewee) => interviewee.user_id === record.user_id);
						// 	user?.interview_id === undefined && user?.user_id === undefined;
						// }}
	...
	}, [handleShowResults, intervieweeListInfo, getResultByUserIdInterviewIdProgress]);
	...
	const fetchPdf = async () => {
		let id;
		if (selectedInterviewee?.user_id) id = selectedInterviewee?.user_id;
		else id = selectedInterviewee?._id;
		try {
			const response = await fetch(`${import.meta.env.VITE_BACKEND_BASE_URL}/api/user/individual_user/cv/${id}`, {
				method: "GET",
				headers: {
	...
	console.log("EMRE ALANI", getResultByUserIdInterviewIdInfo?.data?.questions);
	const handleReject = () => {
	...
	useEffect(() => {
		if (updateInterviewStatusProgress && !updateInterviewStatusError && !updateInterviewStatusInfo) {
			toast.loading("Status updating...", toastConfig);
		} else if (updateInterviewStatusError && !updateInterviewStatusProgress) {
			toast.dismiss();
			toast.error("Status update failed!", toastConfig);
		} else if (updateInterviewStatusInfo && !updateInterviewStatusError && !updateInterviewStatusProgress) {
			toast.dismiss();
			toast.success("Status updated!", toastConfig);
		}
	}, [updateInterviewStatusError, updateInterviewStatusInfo, updateInterviewStatusProgress]);
	return (
		<ConfigProvider
			theme={{
					...
					<div className={styles.right_side}>
						...
						<Card loading={!selectedInterview}>
							...
							<div className={`${styles.flex} mb-2`}>
								...
								{tableView === "interviewees" && (selectedInterviewee?.status?.name !== "REJECTED" && selectedInterviewee?.status?.name !== "ACCEPTED") && (
									<>
								...
								<>
									...
									{getResultByUserIdInterviewIdInfo && (
										<List
											...
											bordered
											dataSource={getResultByUserIdInterviewIdInfo?.data?.questions.map((question) => `${question.topic} ${question.isTrue ? "✓" : "ⓧ"}`)}
											locale={{
