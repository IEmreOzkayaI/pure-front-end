...
export default function SignUp() {
	...
	const user = localStorage.getItem("user_type") ? localStorage.getItem("user_type") : location.state.type;
	...
	const handleOnSubmit = (e) => {
		e.preventDefault();
		if (user === "user") {
			if (!firstName || !lastName || !phoneNumber || !email || !password || !repeatPass || !terms) {
				setError({
					firstName: !firstName,
					lastName: !lastName,
	...
	return (
		<motion.div>
			<div className={styles.container}>
				<Navbar />
				<div className={styles.grid} style={user === "user" ? {gridAutoRows: 65 + "rem"} : {}}>
					<div className={styles.explanation}>
						...
						<div className={styles.image}>
							<svg xmlns='http://www.w3.org/2000/svg' width='800' height='513' viewBox='0 0 800 513' fill='none'>
								<path
									fillRule='evenodd'
									clipRule='evenodd'
								...
								<>
									<div className={styles.col}>
										<div className={styles.inputGroup}>
											...
											/>
											{error.companyName ? <span className={styles.errorMessage}>Field is required</span> : null}
										</div>
										<div className={styles.inputGroup}>
											...
											/>
											{error.industry ? <span className={styles.errorMessage}>Field is required</span> : null}
										</div>
									</div>
									<div className={styles.col}>
										<div className={styles.inputGroup}>
											...
											/>
											{error.vat ? <span className={styles.errorMessage}>Field is required</span> : null}
										</div>
									</div>
								</>
							) : null}
								...
								<div className={styles.inputGroup}>
									...
									<input
										...
										onChange={(e) => {
											...
											setError({
												...error,
												email: !e.target.value.match(/^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/),
											});
										}}
										onBlur={() => {
											setError({
												...error,
												email: !email || !email.match(/^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/),
											});
										}}
									/>
									{error.email ? <span className={styles.errorMessage}>Please enter a valid email</span> : null}
								</div>
							</div>
							<div className={styles.col}>
								...
								<div className={styles.inputGroup}>
									...
									<input
										...
										onChange={(e) => {
											...
											setError({
												...error,
												password: !/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,10}$/.test(e.target.value),
											});
										}}
										onBlur={() => {
											setError({
												...error,
												password: !password.match(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,10}$/),
											});
										}}
									...
									/>
									{error.password ? <span className={styles.errorMessage}>Field is required or doesn&apos;t meet criteria</span> : null}
								</div>
								<div className={styles.inputGroup}>
									...
									<input
										...
										onChange={(e) => {
											...
											setError({
												...error,
												repeatPass: !repeatPass || repeatPass.match(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,10}$/),
											});
										}}
										onBlur={() => {
											setError({
												...error,
												repeatPass: !repeatPass.match(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,10}$/),
											});
										}}
										pattern='^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,10}$'
										maxLength={10}
										...
										title='Minimum eight and maximum 10 characters, at least one uppercase letter, one lowercase letter, one number and one special character'
									/>
									{error.repeatPass ? <span className={styles.errorMessage}>Field is required or doesn&apos;t match password</span> : null}
								</div>
							</div>
							<div className={styles.col}>
								<div className={`${styles.inputGroup} ${styles.privacy}`}>
									...
									/>
									<label htmlFor='terms'>
										I’ve read and agree to the <Link to='privacy-policy'>privacy policy</Link>
									</label>
									{error.terms ? <span className={styles.errorMessage}>Field is required</span> : null}
								</div>
							</div>
