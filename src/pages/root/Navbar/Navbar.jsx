		...
		<nav className={styles.navbar}>
            ...
            <li>
              <Link to={"/interview/playground"} target="_blank">
				...
				{user?.role !== "Individual_User" && window.location.href.includes("/dashboard") && (
					<>
						<Button className={["dark"]} style={{alignSelf: "center", marginLeft: "auto", marginRight: "5rem", padding: "0.5rem 1rem", fontSize: "1.2rem", fontWeight: "100", border: "none"}} onClick={() => setIsModalOpen(true)}>
							Add Interview
						</Button>
						<CustomModal width={"100%"} isOpen={isModalOpen} setIsOpen={setIsModalOpen} label={"Interview Creation Form"}>
							<div>
							...
							<div className={styles.dropdownContentLoggedIn}>
								<div>
									<div className={styles.imgContainer}>
										{user.role === "Individual_User" && <img src='./avatar.png' alt='' />}
										{user.role === "Company_User" && <img src='./building.png' alt='' />}
									</div>
									<div>
										{user.name} {user.surname}
									</div>
						...
						<>
							...
							<li>
								...
								<div className={styles.dropdownContentSignUp}>
									<Link className={styles.dropdownItem1} to={"/signUp"} state={{type: "company"}} onClick={() => localStorage.setItem("user_type", "company")}>
										<img src='./building.png' alt='company' /> Companies
									</Link>
									<Link className={styles.dropdownItem2} to={"/signUp"} state={{type: "user"}} onClick={() => localStorage.setItem("user_type", "user")}>
