...
export default function Login() {
	...
	useEffect(() => {
		if (logInInfo) {
			dispatch(userFetch(logInInfo.access_token));
		}
	}, [logInInfo]);

	useEffect(() => {
		if (logInError.message === "User is not active") {
	...
	return (
		<motion.div>
								...
								<div className={styles.inputGroup}>
									...
									<input
										...
										onBlur={() => {
											setError({
												...error,
												email: !logInForm.email || !logInForm.email.match(/^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/),
											});
										}}
									/>
								...
								<div className={styles.inputGroup}>
									...
									<input
										...
										onBlur={() => {
											setError({
												...error,
												password: !logInForm.password.match(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,10}$/),
											});
										}}
										minLength={8}
									/>
			...
			<div>
				{confirmModal && <ConfirmModal handleModal={handleModal} redirectPath={"/dashboard"} />}
				<Footer />
			</div>
		</motion.div>
	);
